...
simulation::simulation(SubWindow &subWindow, MainWindow &mainWindow) :
...
void simulation::addmatch(Team& team, Team& team1, Team& team10)
{
...
void simulation::addresult(Team& team, Team& team1)
{
        ...
        for (h = 0; h < 2; h++)
        {
            ...
            for (n = 0; n < 19; n++)
            {
                ...
                for(int k = 0; k < 20; k++)
                {
                    if (k == n)
                    {
                        team3 = DataBase::Teams[n];
                        addmatch(DataBase::Teams[n], team3, team10);
                    }
                }
                ...
                std::sort(DataBase::Clone.begin(), DataBase::Clone.end(), less_than_key());
                mainWindowRef.printtable();
                subWindowRef.roundcounter++;
                subWindowRef.IncrementRound(subWindowRef.roundcounter);
                if (a == 190)
                    break;
            }
        }
        subWindowRef.SetWinners();
        if (DataBase::Clone[0].name == "My Team")
        {
            subWindowRef.WinnerMessage();
        }
        subWindowRef.SetVisibleRetry(true);
        Player p;
        p.SetAge();
}

Team simulation::Simulation(Team& team1, Team& team2)
{
    srand(time(NULL));
...
Team simulation::SimulateMatch(Team& team1, Team& team2)
{
    ...
    ClonedTeam2.wins = team2.wins;
    ClonedTeam2.draws = team2.draws;
    ClonedTeam2.losses = team2.losses;
    ClonedTeam2.points = team2.points;
